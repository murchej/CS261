...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    while (current){        //if current's value matches the query value, return the value of current
    ...
    return -1; //return -1 if we go all the way though the list and no element matched val because val doesnt exist
}
...
void list_reverse(struct list* list)
{
    ...
    struct node* next, * current = list->head, * prev = NULL;
