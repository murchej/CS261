...
struct dynarray* dynarray_create()
{
    ...
    da->data = malloc(DYNARR_INITCAP * sizeof(void*));
    
    assert(da->data);

    da->size = 0;
    
    da->capacity = DYNARR_INITCAP;
...
void dynarray_insert(struct dynarray* da, void* val)
{
     ...
     if(da->size == da->capacity){
         ...
         da->capacity *= 2; //doubles capacity if needed

         void** newarr = malloc(da->capacity * sizeof(void*));
    ...
    do{                                //puts new element at the end of the array

        da->data[da->capacity-1] = val;

    }while(da->data[da->capacity-1] != val);
