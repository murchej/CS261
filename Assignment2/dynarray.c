...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  for (int i = 0; i < da->size; i++) {
    ...
    da->physical = (da->start + i) % da->capacity;

    new_data[i] = da->data[da->physical];
  ...
  free(da->data);
  da->data = new_data;
  da->capacity = new_capacity; //realigns array data
  da->start = 0;
}
...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   */
  da->physical = (da->start + da->size) % da->capacity;
  da->data[da->physical] = val;
  da->size++;
}
