 ...
 */
void list_remove(struct list* list, void* val) {
  assert(list);

  struct node* prev = NULL, * curr = list->head;

  int compare = strcmp(val, curr->val); //allows me to call function easier
...
void* node_value(struct node* node){

  return node->val;
}

void* next_node(struct node* curr){
