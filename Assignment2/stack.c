...
struct stack* reverse_stack(struct stack* stack){ //reverses stack and returns a reversed stack
	
	struct stack* stack_copy = stack_create();

	struct list* temp = stack->list;

	struct node* curr = list_head(temp);

	while(curr != NULL){

		list_insert(stack_copy->list, node_value(curr));
...
void* stack_pop(struct stack* stack) {
	...
	void *val = head_value(stack->list);

	list_remove(stack->list, val);
